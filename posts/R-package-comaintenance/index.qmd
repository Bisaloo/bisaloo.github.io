...
But collaborative open-source work can with unique challenges because some open-source projects happen outside of the transactional relationship inherent to bussinesses.
Hierarchies and processes are thus not as clearly defined as they might be in a company.
...
However, I will not be exhaustive, as this has been treated elsewhere, and because I believe the solutions cannot be detached from the environment in which they are implemented.
...
## What do I mean by co-maintenance?

By co-maintainers, I mean individuals that all have write access to the source code repository.
...
Many projects I've been in have been bitten by underestimating the important of choosing a future-proof communication channel.
...
Another example is the lightr R package I'm maintaining, where we sometimes discuss proprietary documentation that was shared in confidence by spectrometer manufacturers.
...
In low-resource or small-scale projects, I do not believe it should always be a hard requirements to review every single pull requests.
...
### Bonus: streamline communication and avoir uncertainty
...
A good example of an attempt to explicit these processes to enable collaboration at-scale in a project involving multiple universities across the world are the [Epiverse blueprints](https://epiverse-trace.github.io/blueprints).
...
R packages with a specific set of constraints, mostly due to CRAN requirements and submission process.
...
So, even if in practice, you have chosen to share the maintenance of the package on a equal footing, only one person can communicate directly with CRAN.

In particular, only one person will receive the emails about failing checks and archival threats.
This is particularly difficult because (as of writing this post) CRAN usually gives short deadlines and maintainers usually only have a couple of week to resubmit.
...
In this specific situation, the "official" maintainer, signaled by "`cre`" in the `DESCRIPTION`, is a bottleneck at two moments:
...
I have developed two GitHub Actions workflows which can alert of watchers from a given GitHub repository that the package is at risk of archival.
...
The following action placed in the repository of an R package will open a new issue each time a new archival deadline is set by the CRAN:
...
- an archival risk alert system, where each package archival risk is tracked in a given issue that co-maintainers can subscribed to
...
However, this was mentioned in passing as something that might happen eventually but no promise was made not any clear timeline established.